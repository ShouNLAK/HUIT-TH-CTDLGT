...
int insertAVLNode(AVLNode *&T, AVLNode *p)
{
	...
	int Result;
	if (StringCompare(T->Info.Word,p->Info.Word) == 0)
		return 0;
	if (StringCompare(T->Info.Word, p->Info.Word) == 1)
	{
...
int deleteAVLNode(AVLNode *&T, ItemType x)
{
	int Result;
	if (T == NULL) return 0;
	if (StringCompare(T->Info.Word, x.Word) == 1)
	{
	...
	else if (StringCompare(T->Info.Word, x.Word) == -1)
	{
...
{
	if (!root) return NULL;
	if (StringCompare(root->Info.Word, x.Word) == 0) return root;
	else if (StringCompare(root->Info.Word, x.Word) == 1)
